...
export function ensureButtonElement(
	selector: string,
	context?: HTMLElement
): HTMLButtonElement {
...
export function ensureInputElement(
	selector: string,
	context?: HTMLElement
): HTMLInputElement {
...
export function ensureImageElement(
	selector: string,
	context?: HTMLElement
): HTMLImageElement {
...
export function ensureTemplateElement(
	selector: string,
	context?: HTMLElement
): HTMLTemplateElement {
...
export function cloneTemplate<T extends HTMLElement>(
	...
	if (typeof query === 'string') {
		...
		if (!(element instanceof HTMLTemplateElement)) {
			throw new Error(
				`Element found with selector "${query}" is not a template`
			);
		}
		template = element;
	} else {
	...
	if (!template.content || template.content.children.length === 0) {
		throw new Error(
			`Template content is empty for selector "${template.id || 'unknown'}"`
		);
	}
...
export function safeEnsureElement<T extends HTMLElement>(
	selector: string,
	context: HTMLElement,
	defaultValue?: T
): T | null {
...
export function safeEnsureAllElements<T extends HTMLElement>(
	selector: string,
	context: HTMLElement = document as unknown as HTMLElement
): T[] {
	try {
		return ensureAllElements<T>(selector, context);
	} catch (error) {
